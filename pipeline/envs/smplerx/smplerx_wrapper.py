...
def load_model(weights_path: Path, model_name: str):
    ...
    # cfg.encoder_config_file est relatif au dossier `main/` du repo SMPLer-X
    # (cwd attendu par leur workflow officiel : `cd repo/main && python ...`).
    # Notre CWD est différent, donc on convertit en absolu si ce n'est pas déjà fait.
    if hasattr(cfg, "encoder_config_file"):
        enc_path = Path(cfg.encoder_config_file)
        if not enc_path.is_absolute():
            cfg.encoder_config_file = str(SMPLERX_REPO / "main" / enc_path)
